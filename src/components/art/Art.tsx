/**
 * Line drawings in the NNG style: a shared stroke family, Verdigris ink, no ornamental fills. The four service drawings
 ...
 * in the first time it scrolls into view (Motion.tsx), then settles. Reduced motion and PDF
 * exports show every diagram complete without animation.
 */

type ArtProps = { className?: string };

const svgProps = { fill: "none", strokeLinecap: "round", strokeLinejoin: "round", "aria-hidden": true, focusable: "false", "data-draw": "" } as const;
...
/** Alignment: three points sharing one direction, not a second Vastu floor plan. */
export function AlignmentArt({ className }: ArtProps) {
  return (
    <svg viewBox="0 0 120 120" className={className} {...svgProps} data-art="alignment">
      <path {...L} d="M20 96 100 16M86 16h14v14" />
      <circle {...L} cx="34" cy="82" r="13" />
      <circle {...L} cx="60" cy="56" r="13" />
      <circle {...L} cx="86" cy="30" r="13" />
    </svg>
  );
}
...
export function AreasFigure({ className = "areas-figure" }: ArtProps) {
  ...
  return (
    <svg viewBox="0 0 320 320" className={className} role="img" aria-label="One life at the centre, with health, relationship, career and money around it" data-draw="" data-art="areas">
      <g fill="none" stroke="var(--art)" strokeWidth="1.4">
