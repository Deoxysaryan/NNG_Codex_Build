...
export function Header() {
  ...
  useEffect(() => {
    ...
    const background = [...document.querySelectorAll<HTMLElement>("main, .site-footer, .mobile-enquiry, .contact-dock")];
    ...
    const onKey = (event: KeyboardEvent) => {
      ...
      if (event.key === "Tab") {
        const items = [toggle.current, ...Array.from(sheet?.querySelectorAll<HTMLElement>("a[href], button") ?? [])].filter((el): el is HTMLElement => !!el);
        const first = items[0];
